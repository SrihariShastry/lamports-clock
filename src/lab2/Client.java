...
public class Client {
	...
	public boolean startClient() {
		...
		try
		{
			...
			// if failed to connect, show appropriate message to the client
	...
	/*
	 * Continuously wait for reply from server and parse response 
	 */
		...
		public void run() {
			...
			while(true) {
				...
				try {
					...
					String type = response[3].substring(13);
					...
					else if(contentType.trim().equalsIgnoreCase("time")) {
//						clientGUI.updateClientLog(response[6]);
						clientGUI.updateClock(response[5]);
					}
					//if client list
					else {
					...
					// connection failed. 
					showMessage("Server has closed the connection: " + e);
					if(clientGUI != null) 
