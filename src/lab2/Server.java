...
//	}
	...
	public void startServer() {
		...
		try 
		{
			...
			while(runT) 
			{
				...
				//start the thread and listen to it continuously 
				cThread.start();
			}
	...
	class CThread extends Thread {
		...
		/*
		 * Continuously listen to the client requests
		 * input	: none
		 * output 	: none
		 */
		public void run() {
			// Continuously listen to the cient for any requests
			boolean runT = true;
			while(runT) {
				...
				try {
					...
					if(request[3].contains("time")) {
						response[2] = "Content-type: time";
					...
					else if(request[0].contains("GET")) {
